...
export function PostDetail({ post }) {
  ...
  const { isLoading, isError, data, error } = useQuery(['comments', post.id], () => fetchComments(post.id));
  ...
  return (
    <>
      ...
      {deleteMutation.isSuccess && <p style={{ color: 'blue' }}>Success</p>}
      ...
      {updateMutation.isSuccess && <p style={{ color: 'blue' }}>Success</p>}
      <p>{post.body}</p>
      <h4>Comments</h4>
      {data.map((comment) => (
        <li key={comment.id}>
